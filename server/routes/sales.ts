  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        s.tax_amount,
        s.discount_amount,
        COALESCE(s.loading_fee, 0) as loading_fee,
        s.grand_total,
        ...
        s.cashier_id,
        COALESCE(s.cashier_name, u.name, 'Imtiaz Ali') as cashier_name,
...
salesRouter.post('/', (req: Request, res: Response): any => {
  try {
    ...
    const resolvedCashierName = (typeof cashier_name === 'string' && cashier_name.trim().length > 0)
      ? cashier_name.trim()
      : (cashierUser?.name || 'Imtiaz Ali');
    ...
    const loadingFee = Math.max(0, Math.round((Number(loading_fee) || 0) * 100) / 100);
    const grandTotal = Math.max(0, Math.round((calculatedSubtotal - orderDiscount + orderTax + loadingFee) * 100) / 100);
    ...
    try {
      ...
      const saleResult = db
        .prepare(`
          INSERT INTO sales (
            invoice_number, branch_id, shift_id, cashier_name, customer_id, sale_date, subtotal, tax_amount, discount_amount, loading_fee,
      ...
      `).run(
        session.userId,
        saleId,
        `Created invoice ${invoiceNumber} at ${branch.name} for ${customer.name}. Cashier: ${resolvedCashierName}. Grand Total: Rs. ${grandTotal.toLocaleString()}`
      );
...
// PATCH /api/sales/:id/loading-fee - Update loading/unloading fee for an existing invoice
salesRouter.patch('/:id/loading-fee', (req: Request, res: Response): any => {
  try {
    const saleId = Number(req.params.id);
    const newFee = Math.max(0, Math.round((Number(req.body.loading_fee) || 0) * 100) / 100);
    ...
    const currentFee = Number(sale.loading_fee || 0);
    const diff = newFee - currentFee;

    const newGrandTotal = Math.max(0, Math.round((sale.grand_total + diff) * 100) / 100);
    const newNetTotal = Math.max(0, Math.round(((sale.net_total || sale.grand_total) + diff) * 100) / 100);
    const newOriginalTotal = Math.max(0, Math.round(((sale.original_grand_total || sale.grand_total) + diff) * 100) / 100);
    const newDue = Math.max(0, Math.round((newGrandTotal - sale.paid_amount) * 100) / 100);
    ...
    try {
      db.prepare(`
        ...
        SET loading_fee = ?,
            ...
            payment_status = ?
        WHERE id = ?
      `).run(newFee, newGrandTotal, newOriginalTotal, newNetTotal, newDue, newPaymentStatus, saleId);
      ...
      if (sale.customer_id) {
        db.prepare(`
          ...
          SET total_purchases = total_purchases + ?,
              ...
              updated_at = CURRENT_TIMESTAMP
          WHERE id = ?
        `).run(diff, diff, sale.customer_id);
      }
      ...
      return res.json({
        success: true,
        message: 'Loading fee updated successfully.',
        data: {
          loading_fee: newFee,
    ...
    console.error('Error updating loading fee:', err);
    return res.status(500).json({ success: false, message: 'Failed to update loading fee.' });
  }
});
